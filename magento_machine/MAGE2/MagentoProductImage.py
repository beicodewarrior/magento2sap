...
# [GCC 7.5.0]
# Embedded file name: /opt/magedst/MAGE2/MagentoProductImage.py
...
class MagentoProductImage(MagentoProduct):
    ...
    def downloadImage(self, imageUrl, forceDownload=False):
        downloadResult = {'image_url': imageUrl, 'http_response_code': None, 
           'file_name': None, 
           'success': False, 
           'log': ''}
        try:
    ...
    def createProductImageThroughAPI(self, eProductImage):
        ...
        syncResult = {'sku': sku, 'http_response_code': None, 
           'media_url': None, 
           'sync_status': 'F', 
           'sync_notes': ''}
        try:
            ...
            if downloadResult['success']:
                ...
                imageFileEntity = {'content': imageData, 'mime': 'image/jpeg'}
                productImageEntity = {'file': imageFileEntity, 'label': eProductImage['image_label'], 
    ...
    def assignImageToProduct(self, eProductImage, filename, imageType, duplicateImage=False):
        syncResult = {'sku': eProductImage['sku'], 'media_url': None, 
           'sync_status': 'F', 
           'sync_notes': ''}
        ...
        if productId == 0:
            ...
            if imageType in self._singleImageTypes:
                insParam = [attributeMetadata['attribute_id'], 0, productId, addedFilename, addedFilename]
                self.mageCursor.execute(self.queries['insertProductImageSQL'], insParam)
            if duplicateImage == False:
                mediaGalleryParam = [mediaGalleryAttributeMetadata['attribute_id'], addedFilename, 'image']
                ...
                if self._needAssignLabel == True:
                    ...
                    label = None
                mediaGalleryValueParam = [valueId, 0, productId, label, position]
