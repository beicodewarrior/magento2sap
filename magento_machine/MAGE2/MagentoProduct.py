...
# [GCC 7.5.0]
# Embedded file name: /opt/magedst/MAGE2/MagentoProduct.py
...
class MagentoProduct(MagentoCore):
    ...
    def __init__(self, mageConf, mageConn=None, mageApi=None, dstCursor=None):
        ...
        self.productDefaults = {'attribute_set_name': 'Default', 'website_code': 'admin', 
           'store_code': 'admin', 
           'status': '1', 
           'type_id': 'simple', 
           'visibility': '4', 
        ...
        self._entityLogConf = {'enableLog': True, 'logPath': self.mageConf['logPath'] + '/entity_log/', 
           'logFileName': ''}
        self._inventoryDefaultConfig = {'min_qty': 0, 'use_config_min_qty': 1, 
           'max_sale_qty': 10000, 
        ...
        self._categoryEavDataDefaults = {'is_active': '1', 'is_anchor': '1', 
           'include_in_menu': '0', 
    ...
    def getWebsiteCodesByStoreCodes(self, storeCodes):
        ...
        if len(websiteCodes) == 0:
            websiteCodes = [self.productDefaults['website_code']]
    ...
    def scheduleImages(self, productId, formattedProductJsonObj):
        sku = formattedProductJsonObj['sku']
        syncResult = {'sku': sku, 'product_id': productId, 
           'sync_status': '', 
           'sync_notes': ''}
        try:
            imageTypes = [
             'image', 'small_image', 'thumbnail', 'swatch_image', 'slider_image', 'media_gallery']
            mainImage = ''
            mainImagePosition = 0
            position = 0
            imageLabel = ''
    ...
    def assignStock(self, productId, formattedProductJsonObj):
        sku = formattedProductJsonObj['sku']
        syncResult = {'sku': sku, 'product_id': None, 
           'sync_status': 'F', 
    ...
    def importInventory(self, product):
        sku = product['sku']
        syncResult = {'id': product['id'], 'action': '', 
           'sku': sku, 
           'product_id': None, 
           'sync_status': 'F', 
           'sync_notes': 'DST to Magento'}
    ...
    def importInvenoryExt(self, productId, product, syncResult):
        syncResult = {'sync_status': 'O', 'sync_notes': ''}
        return syncResult
    ...
    def importProductPrice(self, product):
        ...
        self.stores = self.getStores()
        sku = product['sku']
        syncResult = {'id': product['id'], 'sku': sku, 
           'product_id': None, 
           'sync_status': 'F', 
           'sync_notes': 'DST to Magento'}
        productId = self.getProductIdBySku(sku)
    ...
    def getRootCategoryByStore(self, storeCode):
        ...
        rootCategory = {'id': rootCategoryId, 'value': rootCategoryValue}
        return rootCategory
    ...
    def createCategoryThrougAPI(self, currentPathIds, category):
        ...
        childrenCount = 0
        position = 0
        categoryData = {'category': {'parentId': parentId, 'name': category, 
                        'isActive': self._categoryEavDataDefaults['is_active'], 
                        'position': position, 
                        'level': level, 
                        'children': childrenCount, 
                        'availableSortBy': [], 'includeInMenu': self._categoryEavDataDefaults['include_in_menu'], 'extensionAttributes': {}, 'customAttributes': []}}
        mCategory = self.mageApi.createCategory(categoryData)
        return mCategory['id']

    def linkCatalogCategory(self, sku, categoryId):
        productCategoryData = {'productLink': {'sku': sku, 'categoryId': categoryId}}
    ...
    def importProductCategory(self, eProductCategory, througApi=False):
        ...
        syncResult = {'id': eProductCategory['id'], 'action': action, 
           'sku': sku, 
           'path': path, 
           'sync_status': 'F', 
           'sync_notes': 'DST to Magento'}
    ...
    def refreshUrlRewriteThroughAPI(self, productId, mProduct):
        ...
        if productData is None:
            ...
            urlKeyAttributeData = {'attribute_code': 'url_key', 'value': urlKey}
